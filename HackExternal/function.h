...
DWORD DistanceColor(bool)
{

	ImColor Color;
		Color = CFG.DistanceColor;
...
Vector3 ProjectWorldToScreen(Vector3 WorldLocation)
{
	...
	auto RangedWeaponComponent = read<DWORD_PTR>(GameVars.local_player_pawn + GameOffset.RangedWeaponComponent);
	auto bIsAiming = read<bool>(RangedWeaponComponent + GameOffset.bIsAiming);
	if (bIsAiming)
	{
		auto EquippedWeaponType = read<DWORD_PTR>(RangedWeaponComponent + GameOffset.EquippedWeaponType);
		auto AimedFov = read<int32_t>(EquippedWeaponType + GameOffset.AimedFov);
		FovAngle = AimedFov;
	}
	else
		FovAngle = 100;
