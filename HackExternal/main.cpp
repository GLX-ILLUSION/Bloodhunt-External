...
auto CallHacks()->VOID
{
	while (true)
	{
		if (CFG.b_NoRecoil)
		{
			...
			write<float>(GameVars.equipped_weapon_type + 0x110, 1.f); // HorizontalRecpo;Curve
			write<float>(GameVars.equipped_weapon_type + 0x108, 1.f); // VerticalRecoilCurve
			...
			if (entity_distance < CFG.max_distance)
			{
				...
				if (CFG.b_TeamCheck)
				{
					...
					if (mode == 9)// if mode 9 its team mate
					{
						...
						sprintf_s(ID, "Teammate", Group_ID);
				...
				//if (CFG.b_Fastres)
				//	read<DWORD_PTR>(GameVars.local_player_pawn);
				//	write<float>(BloodHunt::Variables::local_player_pawn + 0xcf0, 1337);
			}
		}
	}
}
void ToggleButton(const char* str_id, bool* v)
{
	...
	if (colors)
	{
		col_bg = IM_COL32(70, 150, 255, CFG.Transparency * 150); // Hoverover
		if (!ImGui::IsItemHovered())
			col_bg = *v ? IM_COL32(0, 255, 0, CFG.Transparency * 150) : IM_COL32(255, 0, 0, CFG.Transparency * 150); // True/false
	}
	else
	{
		col_bg = ImColor(100, 100, 100, CFG.Transparency * 200); // Hoverover
		if (!ImGui::IsItemHovered())
			col_bg = *v ? IM_COL32(0, 255, 0, CFG.Transparency * 200) : IM_COL32(255, 0, 0, CFG.Transparency * 200); // True/false
		else
	...
	DrawString(ImVec2(p.x + 40, p.y + 2), str_id, IM_COL32(218, 218, 218, 255));
}
void InputHandler() {
...
void Render()
{
	...
	if (CFG.b_MenuShow)
	{
		...
		ImGui::Begin("Zonia.xyz", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		ImGui::Separator(); // Shows cheat fps + ms delay
		float MAX_FPS = ImGui::GetIO().Framerate;
		ImGui::Text("External %.3f MS/FPS (%.1f FPS)", 1000.0f / MAX_FPS, MAX_FPS);
		ImGui::Separator();
		...
		if (CFG.tab_index == 0)
		{
			ToggleButton("Enabled Visual", &CFG.b_Visual);
			if (CFG.b_Visual)
			{
				ToggleButton("ESP Box", &CFG.b_EspBox);
				if (CFG.b_EspBox)
				{
				...
				ToggleButton("Item Esp", &CFG.b_Itemesp);
				ToggleButton("Team Check", &CFG.b_TeamCheck);
				ToggleButton("ESP Skeleton", &CFG.b_EspSkeleton);
				ToggleButton("ESP Line", &CFG.b_EspLine);
				if (CFG.b_EspLine)
				{
					ImGui::Combo("ESP Line Type", &CFG.LineType, CFG.LineTypes, 3);
				}
				ToggleButton("ESP Name", &CFG.b_EspName);
				ToggleButton("ESP Distance", &CFG.b_EspDistance);
				ToggleButton("ESP HealthBar", &CFG.b_EspHealth);
				...
				if (ImGui::ColorEdit3("Name Color", CFG.fl_NameColor, ImGuiColorEditFlags_NoDragDrop))
				{
					CFG.VisibleColor = ImColor(CFG.fl_NameColor[0], CFG.fl_NameColor[1], CFG.fl_NameColor[2]);
				}
				if (ImGui::ColorEdit3("Distance Color", CFG.fl_DistanceColor, ImGuiColorEditFlags_NoDragDrop))
				{
					CFG.VisibleColor = ImColor(CFG.fl_DistanceColor[0], CFG.fl_DistanceColor[1], CFG.fl_DistanceColor[2]);
				}
			}
		}
		else if (CFG.tab_index == 1)
		{
			ToggleButton("Enabled Aimbot", &CFG.b_Aimbot);
			if (CFG.b_Aimbot)
			{
				ToggleButton("Show FOV", &CFG.b_AimbotFOV);
				if (CFG.b_AimbotFOV)
				{
				...
				ImGui::SliderFloat("Smoothing Value", &CFG.Smoothing, 1.0f, 100.0f);

			}
		}
		else if (CFG.tab_index == 2)
		{
			ImGui::Text("These Are Risk Features");
			{
				ToggleButton("Enabled Risk", &CFG.b_Risk);
				if (CFG.b_Risk)
				{
					ToggleButton("No Recoil", &CFG.b_NoRecoil);
					ToggleButton("No Spread", &CFG.b_NoSpread);
					ToggleButton("Fast Revive", &CFG.b_Fastres);
					ToggleButton("Fast Reload | Stays Active Till lobby", &CFG.b_Fastreload);
					ToggleButton("Rapid Fire  | Stays Active Till lobby", &CFG.b_Rapidfire);
					ImGui::SliderFloat("Fire Rate", &CFG.Firerate, 1.0f, 100.0f);
				}
			}
		}
		ImGui::PopFont();
